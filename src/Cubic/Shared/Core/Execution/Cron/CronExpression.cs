...
namespace Cubic.Core.Execution.Cron
{
  public class CronExpression
  {
    ...
    public bool IsDueOrPast(DateTime time)
    {
      return new CronExpressionPart(this._minutes, 60).IsDueOrPast(time.Minute)
             && new CronExpressionPart(this._hours, 24).IsDueOrPast(time.Hour)
             && new CronExpressionPart(this._days, 31).IsDueOrPast(time.Day)
             && new CronExpressionPart(this._months, 12).IsDueOrPast(time.Month)
             && new CronExpressionPart(this._weekdays, 7).IsDueOrPast((int)time.DayOfWeek);
    }
  }
}