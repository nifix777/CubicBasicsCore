...
namespace Cubic.Core.Execution.Cron
{
  /// <summary>
  ...
  public class CronExpressionPart
  {
    ...
    /// <param name="expression"></param>
    /// <returns></returns>
    private IEnumerable<int> GetCronIntArray(string expression, bool exactTime = true)
    {
      ...
      {
        return GetSpecifiedInt(expression, _timeParts, exactTime);
      }
    }
    ...
    /// <returns></returns>
    private static IEnumerable<int> GetSpecifiedInt(string expression, int timeParts, bool exactTime)
    {
      ...
      if (!exactTime)
      {
        if (timeParts != 0)
        {
          ...
          {
            if (i >= parsedValue) yield return i;
          }
        }
        else
        {
    ...
    public bool IsDueOrPast(int time)
    {
      return this.CronExpressionPartIsDueOrPast(time, this._expression, this._timeParts);
    }

    private bool CronExpressionPartIsDueOrPast(int time, string expression, int replaceZeroWith)
    {
      if (expression == "*")
      {
        return true;
      ...
      {
        var values = this.GetCronIntArray(expression, false);
        return values.Contains(time);
      }
    }
  }
