...
namespace Cubic.Core.Tools
{
  ...
  public class SemanticVersion : IEquatable<SemanticVersion>
  {
    ...
    {
      Guard.AgainstNullOrEmpty(semanticVersion, nameof(semanticVersion));
    ...
    public static SemanticVersion Parse(string semanticVersion)
    {
      ...
      Guard.EnsuresArgument(TryParse(semanticVersion, out result), nameof(semanticVersion), "Unrecognized or unsupported semantic version.");
      return result;
    }
    ...
    {
      Guard.AgainstNull(pattern, nameof(pattern));
      ...
      Guard.EnsuresArgument(pattern.IsMatch(input), parameterName, $"The prerelease must match the pattern \"{pattern}\".");
    }
  }
}
