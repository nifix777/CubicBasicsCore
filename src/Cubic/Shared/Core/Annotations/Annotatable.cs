...
namespace Cubic.Core.Annotations
{
  ...
  public class Annotatable : IMutableAnnotatable
  {
    ...
    {
      return this.FindAnnotation(name) != null;
    }
    ...
    protected virtual Annotation AddAnnotation( [NotNull] string name , [NotNull] Annotation annotation )
    {
      ...
      {
        throw new InvalidOperationException( string.Format("Duplicate Annotation: {0}", name) );
      }
    ...
    public void Copy(IMutableAnnotatable targetMutableAnnotatable)
    {
      foreach (var annotationKv in _annotations.Value)
      {
        targetMutableAnnotatable[annotationKv.Key] = annotationKv.Value;
      }
    }
